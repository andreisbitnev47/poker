...
class Tournament():
    ...
    def rearrangePlayers(self):
        ...
        for tableNr, table in enumerate(self.tables):
            ...
            for playerNr, player in enumerate(tablePlayers):
                if player.getData("stack") <= 0:
                    tablePlayers[playerNr].updateData("stack", -1)
                else:
        ...
        for i in range(tableCnt):
            if math.ceil(activePlayersCnt / (i + 1)) <= self.playersPerTable:
                tableCnt = i + 1
                break
        ...
        freePlayers = [player for player in freePlayers if player.getData("stack") != -1]
        ...
        for index, table in enumerate(self.tables):
            ...
            if tablePlayersMap[index] > minPlayersPerTable:
                ...
                for i, player in enumerate(tablePlayers):
                    if player.getData("stack") != -1 and playersOverMin > 0:
                        freePlayers.append(player)
                        tablePlayers[i].updateData("stack", -1)
        ...
        while len(freePlayers) > 0:
            ...
            for table in self.tables:
                ...
                tablePlayers = [player for player in unsortedTablePlayers if player.getData("stack") != -1]
            self.activePlayers = [player for player in self.players if player.getData("stack") > 0]
            def useStack(player):
            ...
            dqnLost = not [x for x in self.activePlayers if x.getData("index") == self.dqnNr]
...
tournament = Tournament(9, 9, 500, 2);